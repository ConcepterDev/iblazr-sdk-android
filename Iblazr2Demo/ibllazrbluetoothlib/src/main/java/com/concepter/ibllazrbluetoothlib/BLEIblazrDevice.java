...
public class BLEIblazrDevice implements AbstractIBlazrDevice {
    ...
    private static final byte LONG_LIGHT_CHARACTERISTIK = 0x16;
    private static final byte SHORT_LIGHT_CHARACTERISTIK = 0x10;
    private static final byte CHECK_LIGHT_CHARACTERISTIK = 0x11;
    ...
    private void sendValuesToGATT(byte[] values) throws NullPointerException {
        if (!sendingValues || values[0] == LONG_LIGHT_CHARACTERISTIK) {
    ...
    public void light(int timeMode) throws NullPointerException {
        switch (timeMode) {
            case SHORT_LIGHT:
                functionInWritableCharacteristic = SHORT_LIGHT_CHARACTERISTIK;
                sendValuesToGATT(new byte[]{functionInWritableCharacteristic, 0x00, 0x79, temperature, brightness});
                break;
            case LONG_LIGHT:
                functionInWritableCharacteristic = LONG_LIGHT_CHARACTERISTIK;
                sendValuesToGATT(new byte[]{functionInWritableCharacteristic, 0x02, 0, temperature, brightness});
                break;
            case CHECK_LIGHT:
                functionInWritableCharacteristic = CHECK_LIGHT_CHARACTERISTIK;
                sendValuesToGATT(new byte[]{CHECK_LIGHT_CHARACTERISTIK, 0x00, 0x78, 0x3E, 0x08});
                break;
        }
    }
    ...
    public void startLongLight() {
        sendValuesToGATT(new byte[]{LONG_LIGHT_CHARACTERISTIK, 0x00, 0x00, temperature, brightness});
    }

    @Override
    public void stopLongLight() {
        setSendingValues(false);
        sendValuesToGATT(new byte[]{SHORT_LIGHT_CHARACTERISTIK, 0x00, 0x00, temperature, brightness});
    }
